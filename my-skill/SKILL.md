...
- Updated: 2026-02-02
- Language: zh-CN
- Scope: 句法拆解 + 难词语境义 + 翻译产出
- Output Style: 模板化（主干→分层→难点→难词→意群→翻译）
...
- 词/短语：…
  ...
  - 常见意思：…（你最熟的那个义项）
  - 语境义：…（本句真正要表达的义项）
  - 偏译触发线索：…（搭配/介词/宾语/语境态度/学术语域/并列对照等）
  - 与常见意思的关系：…（语义迁移路径：例如“地点→问题/议题”“接触→处理/应对”）
  - 常见搭配/近义替换：…
  - 翻译建议：…（固定译法/意群处理/是否需要意译）
...
#### 熟词偏译迷你示例（用于你输出时对照）
- address（常见：地址）→（语境：处理/解决）
  - 触发：address + problem/concern/issue
  - 关系：从“对某对象说话/致辞”→“对某问题作出回应”→“处理/应对”（语义延伸）
